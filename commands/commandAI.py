    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "你是一位智能编程助手，你叫CodeGeeX。你会为用户回答关于编程、代码、计算机方面的任何问题，并提供 'Markdown格式的规范、可以执行、准确安全、换行标准的代码'注意代码本身不能和markdown语法冲突，如果冲突请解决，并在必要时提供详细的解释。任务：请为输入代码提供格式规范的注释，包含多行注释和单行注释，请注意不要改动原始代码，只需要添加注释。 请用中文回答，并且代码注意换行。"
        ...
        # markdown转换
        html_result = markdown.markdown(result, extensions=['nl2br'])
        image = code_template.render(content=html_result)
        imgkit.from_string(image, 'out.jpg', options={'quality': '75', 'width': '500', 'encoding': "UTF-8"})
